...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'首页','last'=>'末页','theme'=>'  <span><a href="#"> 第%nowPage%页/共%totalPage%页</a></span>
    ...
    public function show($urls) {
        ...
        if ($upRow>0){
            $upPage     =   "<li><a href='".str_replace('__PAGE__',$upRow,$url)."' class='pager-btn'>".$this->config['prev']."</a></li>";
        }else{
            $upPage     =   '';
        }

        if ($downRow <= $this->totalPages){
            $downPage   =   "<li><a href='".str_replace('__PAGE__',$downRow,$url)."' class='pager-btn'>".$this->config['next']."</a></li>";
        }else{
            $downPage   =   '';
        }
        // << < > >>
        ...
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                ...
                if($this->totalPages != 1){
					// 当前页
                    $linkPage .= "<li class=\"active\"><a href=\"javascript:;\" style=\"color:#fff\">".$page."</li>";
                }
            }
        }
		if($this->totalPages>$this->nowPage)
		{
			$linkPage=$linkPage."<li>···</li>";
		}
        $pageStr     =   str_replace(
