                    ...
                    <div class="col-xs-12">
                        <!-- PAGE CONTENT BEGINS -->                        
                            <form class="form-horizontal" role="form" action="<?php echo U('admin/addsort');?>" method="post">
                                <?php echo setParam(array('op'=>'do','sort_id'=>$sort['sort_id'],'p_id'=>$sort['p_id']));?>
                                <?php if(!$sort_id && !$p_id){ ?>
                                ...
                                <div class="form-group">
                                    ...
                                    <select  class="col-sm-12" name="p_id" id='p_id'>
                                        ...
                                        <?php if(is_array($root_sort)): $i = 0; $__LIST__ = $root_sort;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><option <?php if($data['type']!=1){ ?>style="display:none"<?php } ?> root=1 value="<?php echo ($data['sort_id']); ?>" type='<?php echo ($data["type"]); ?>'><?php echo ($data['sort_name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                    </select>                                                                        
                                    </div>
                                    <script type='text/javascript'>
                                        $("select[name='type'] option[value='<?php echo $sort[type] ?>']").attr('selected','selected');
